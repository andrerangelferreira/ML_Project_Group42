...
# Regression models used for model-based feature selection
...
class FeatureSelectionDealer(BaseEstimator, TransformerMixin):
    """
    Custom feature selection transformer compatible with scikit-learn pipelines.

    This class supports multiple feature selection strategies, including:
    - Variance Threshold
    - SelectKBest (statistical tests)
    - Model-based importance selection
    - Recursive Feature Elimination (RFE)
    - Permutation Importance (designed for MLP models)

    The transformer preserves DataFrame structure and can be seamlessly
    integrated into automated preprocessing pipelines.
    """

    def __init__(
        self,
        selection_method="variance",   # Feature selection strategy
        threshold=0.0,                  # for variance threshold
        k=15,                           # Number of features to keep (k-best or model-based)
        model_name= "rf",               # Base model used for model-based methods
        n_features_to_select=None,      # for RFE
        random_state=42,
        n_repeats=10,                   # Permutation importance repetitions
        top_k=None,                     # Top-k features for permutation importance
        threshold_mlp = None,           # Importance threshold for permutation (MLP)
        **kwargs
    ):
    ...
    def get_model(self):
        """
        Returns a regression model based on the selected model_name.
        Used for model-based feature selection and RFE.
        """
    ...
    def fit(self, X, y=None, **kwargs):
        """
        Fits the selected feature selection method to the data.
        """
    ...
    def transform(self, X, y =None, **kwargs):
        """
        Applies the fitted feature selection method and returns
        a reduced DataFrame with selected features only.
        """

        X = X.copy()

        # Variance, KBest and RFE use sklearn selectors
        ...
        # Model-based selection using stored support mask
        elif self.selection_method == "model":
            cols = np.array(X.columns)[self.support_mask_]
            X_sel = X.loc[:, cols]
            return X_sel
        
        # Permutation importance (MLP case)
